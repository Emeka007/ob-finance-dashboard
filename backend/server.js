...
app.get("/callback", async (req, res) => {
  ...
  try {
    ...
    if (!code) return res.redirect(process.env.FRONTEND_URL + "/callback?error=no_code");
    ...
    const userId = uuidv4();
    sessions[userId] = { session_id: sessionId, accounts: accounts, created: Date.now() };
    console.log("Stored session:", userId, "-> session_id:", sessionId, "| accounts:", accounts.length);
    res.redirect(process.env.FRONTEND_URL + "/callback?userId=" + userId + "&session_id=" + sessionId);
  } catch (error) {
